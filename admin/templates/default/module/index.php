					...
					<div class="span6 responsive" data-tablet="span12 fix-offset" data-desktop="span6">
						...
						<div class="portlet box grey">
							<div class="portlet-title">
								<div class="caption"><i class="icon-list"></i>模块列表</div>
								<div class="actions">
									<a href="<?php echo _url('module/index/create', array('menu_id' => $_menuId))?>" class="btn green"><i class="icon-plus"></i> 创建模块</a>
								</div>
							</div>
							<div class="portlet-body">
								<table class="table table-striped table-bordered table-hover" id="table_managed_2">
									...
									<tbody>
										...
										<tr class="odd gradeX">
											<td><input type="checkbox" class="checkboxes" value="<?php echo $k;?>" /></td>
											<td><?php echo $item['name'];?></td>
											<td><?php echo $k;?></td>
											<td><?php echo $item['alias'];?></td>
											<td><?php echo $item['version'];?></td>
											<td><a href="mailto:<?php echo $item['email'];?>"><?php echo $item['team'] ? $item['team'] : '未知';?></a></td>
											<td><?php echo $item['add_time'] ? Date::format(null, $item['add_time']) : '';?></td>
                                            <td>
                                                <?php if (!in_array($k, $sysModuleList)):?>
                                                <?php if ($item['status']):?>
                                                <a href="<?php echo _url('module/index/status', array('module' => $k, 'status' => 0, 'menu_id' => $_menuId));?>" class="btn mini red"><i class="icon-edit"></i> 禁用</a>
                                                <?php else:?>
                                                <a href="<?php echo _url('module/index/status', array('module' => $k, 'status' => 1, 'menu_id' => $_menuId));?>" class="btn mini green"><i class="icon-edit"></i> 启用</a>
                                                 <?php endif;?>
                                                <a href="#" class="btn mini black" onclick="operate.deleteModal($(this));" data-url="<?php echo _url('module/index/uninstall', array('module' => $k));?>"><i class="icon-trash"></i> 卸载</a>
                                                <?php endif;?>
                                            </td>
										</tr>
                                        <?php endforeach;?>
                        ...
                        <div id="modal-delete" class="modal hide fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
                            ...
                            <div class="modal-body">
                                 <p>此项操作将删除模块对应数据库数据以及模块文件夹所有文件，您确认删除吗?</p>
                            </div>
