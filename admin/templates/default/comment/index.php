					...
					<div class="span12 responsive" data-tablet="span12 fix-offset" data-desktop="span12">
						...
						<div class="portlet box grey">
							<div class="portlet-title">
								<div class="caption"><i class="icon-list"></i>评论列表</div>
								<div class="actions">
									<!--<a href="<?php echo _url('comment/index/add');?>" class="btn green"><i class="icon-plus"></i> 添加</a>-->
								</div>
							</div>
							<div class="portlet-body">
								<table class="table table-striped table-bordered table-hover" id="table_managed_2">
									...
									<tbody>
										...
										<tr class="odd gradeX">
											<td><input type="checkbox" class="checkboxes" value="<?php echo $item['comment_id'];?>" /></td>
											<td><?php echo $item['from_name'].' [ID:'.$item['from_id'].']';?></td>
											<td><?php echo $item['module'];?></td>
											<td><?php echo $item['to_name'].'[ID:'.$item['to_id'].']';?></td>
											<td><?php echo $item['content'];?></td>
											<td><?php echo Date::format(null, $item['add_time']);?></td>
                                            <td><?php if ($item['status']):?>
                                                <a class="label label-success"  href="<?php echo _url('comment/index/status', array('id' => $item['comment_id'], 'status' => 0));?>">显示</a>
                                                <?php else:?>
                                                <a class="label label-important" href="<?php echo _url('comment/index/status', array('id' => $item['comment_id'], 'status' => 1));?>">不显示</a>
                                                <?php endif;?>
												<div class="loading"></div>
                                            </td>
                                            <td>
                                                <a href="#" class="btn mini black" onclick="operate.deleteModal($(this));" data-url="<?php echo _url('comment/index/delete', array('id' => $item['comment_id']));?>"><i class="icon-trash"></i> 删除</a>
                                            </td>
										</tr>
                                        <?php endforeach;?>
									</tbody>
